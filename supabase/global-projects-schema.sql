-- ─── BAU Suite — Global Projects Schema ─────────────────────────────────────
-- Multi-user collaborative projects with membership, access codes, and RLS.
-- Run this AFTER the main schema.sql (depends on profiles, set_updated_at()).
--
-- EXECUTION ORDER: Tables → Functions → RLS Policies → Triggers → Indexes
-- ──────────────────────────────────────────────────────────────────────────────
...
-- Allow members to view, AND allow the creator to view their own project
-- (needed for INSERT...RETURNING since the auto-add-creator trigger may not
-- have committed the membership row yet within the same transaction)
