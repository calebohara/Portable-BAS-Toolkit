...
| 49 | add-cascade-soft-delete-rpcs.sql | `cascade_soft_delete_global_project()` + `cascade_soft_delete_project()` SECURITY DEFINER RPCs (atomic parent+child soft-delete cascade) | L (applied 2026-06-11) |
| 50 | add-global-activity-log-server-timestamp.sql | `force_global_activity_timestamp()` BEFORE INSERT trigger (server-owns `global_activity_log.timestamp` so slow-clock devices' rows aren't skipped by the incremental cursor) — P2-3 | L (applied 2026-06-11, checker-verified) |
| 51 | add-last-admin-guard.sql | `prevent_last_admin_removal()` BEFORE DELETE/UPDATE trigger on `global_project_members` (rejects removing/demoting the last admin while other members remain) — P2-2 | L (applied 2026-06-11, checker-verified) |
| 52 | add-activity-log-server-timestamp.sql | `force_activity_timestamp()` BEFORE INSERT trigger (server-owns LOCAL `activity_log.timestamp` — same slow-clock cursor-skip fix as #50, local twin) — MIG-1 | L (applied 2026-06-11, checker-verified) |
| 53 | pin-security-definer-search-path.sql | Pins `search_path = public` on the 4 original SECURITY DEFINER helpers (`is_global_project_member`, `is_global_project_admin`, `join_global_project`, `auto_add_global_project_creator`) — SEC-2 | L (applied 2026-06-11, checker-verified) |
| 54 | harden-project-files-storage-policies.sql | Storage RLS on the `project-files` bucket: UPDATE/DELETE now require `owner = auth.uid()` (they previously checked only `bucket_id`, so any authenticated user could delete or overwrite **any** customer's files); SELECT narrowed to `authenticated` (it had no `to` clause, so `anon` could LIST and enumerate every path). Bucket is still `public = true` — the anonymous *known-URL read* is only closed by the follow-up private-bucket + signed-URL change. — BASAgents 2026-08-29 P0 | L (applied 2026-08-29, checker-verified) |
| 55 | guard-profile-privilege-columns.sql | `guard_profile_privilege_columns()` BEFORE UPDATE trigger on `profiles` rejecting self-service changes to `role` / `approved` / `subscription_tier` / `subscription_expires_at` / `stripe_customer_id` (the self-update policy had no `WITH CHECK`, so any user could PATCH themselves to `role='admin'` and reach the service-role-backed admin API). Also pins `search_path` on `is_admin()` and `handle_new_user()`, which #53 missed. — BASAgents 2026-08-29 P0 | L (applied 2026-08-29, checker-verified) |
| 56 | make-project-files-bucket-private.sql | Sets `project-files` `public = false` so Storage stops serving `/object/public/<path>` without consulting RLS — the other half of #54. **Apply only AFTER deploying the build that uses `getSignedUrl()`**, or file preview/download breaks for clients on the old build (rollback: set `public = true`). — BASAgents 2026-08-29 P0 | **P (pending — deploy app first, then apply)** |
