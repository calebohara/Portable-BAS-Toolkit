...
- **Corrected: metric enthalpy in the Psychrometric Calculator** — SI enthalpy was converted from the imperial value with a scale factor alone, ignoring the different reference datum (imperial is referenced to 0°F dry air, metric to 0°C). Every kJ/kg reading was **17.9 too high** — standard return air showed 65.4 kJ/kg instead of 47.5. Entering a *correct* metric enthalpy in `Dry Bulb + Enthalpy` mode was worse: it came back as ~12% RH instead of 50%, taking dew point, wet bulb and the comfort verdict with it. Differences were unaffected, so coil loads and economizer comparisons were always right — which is why nothing looked wrong. If you work in SI, re-check any saved session where you read an absolute enthalpy.
- **Corrected: humidity ratio at hot, humid conditions** — the calculator silently capped humidity ratio at 0.030 lb/lb, which is well inside real territory: 95°F / 90% RH is 0.033. Entering that returned ~82% RH instead of the 90% you typed. The cap is now the actual saturation limit for the temperature and pressure. (The old fixed value was itself above saturation at 75°F — more moisture than air can physically hold.)
- **Fixed: trend charts hid spikes near the start of a file** — the downsampler skipped the first bucket of every series, so a transient in the opening ~1% of a trend simply never appeared on the chart, and the last point was drawn twice. Also fixed: files with repeated timestamps no longer report a data gap on every sample (`Infinity× normal interval`).
- **Fixed: PPCL Editor could overwrite one program with another** — switching tabs and pressing Ctrl/Cmd+Z reverted the editor to the *previous* program's text and then saved it over the open one. Each document now has its own undo history.
- **Fixed: Terminal no longer stalls on chatty panels** — the full scrollback was being re-serialized to browser storage on every received line, which stalled the UI during a point dump and could exceed the storage quota (after which nothing saved at all). Scrollback is now kept in memory; use Export or Attach to Project to keep a capture.
- **Fixed: dropped telnet/serial connections are now released** — when a panel closed an idle session or a USB-serial adapter was unplugged, the desktop app kept the socket half-open or held the COM port, so reconnecting from a new tab was refused. Connections are now cleaned up when the far end goes away.
- **Fixed: Daily Reports** — Save no longer ignores the Status dropdown (choosing **Finalized** and saving now finalizes); leaving a report with unsaved edits asks first instead of discarding silently; a pending autosave is written out when you navigate away rather than dropped; removing an attachment no longer deletes the photo before you save; and Hours On Site stops overwriting a manual correction each time the report is reopened.
- **Fixed: "Clear File Cache" no longer deletes files that exist only on your device** — it now keeps anything uploaded offline (no cloud copy yet) and all daily-report photos, tells you how many it kept, and the confirmation text says what it actually does. Automatic cleanup when storage runs low follows the same rule, and deleting a file now removes its cloud copy too.
- **Fixed: signing out no longer wipes work you didn't choose to discard** — changing your password signs out your other devices, and that used to clear local data on those devices, taking an offline day's unsynced changes with it. Only a sign-out *you* perform clears local data, and it now warns first if anything hasn't reached the cloud. Signing in as a different user still clears the previous user's data, as before.
- **Fixed: "Sync Now" could strand offline edits** — running a full sync twice over a partly-drained queue deleted the un-pushed items and then skipped them as "unchanged", so those edits never reached the cloud and nothing said so. They are now re-queued.
- **Security: shared file storage locked down** — files in shared projects could be deleted or overwritten by any signed-in user, and the storage bucket could be enumerated anonymously. Access is now restricted to the uploader, and file links are short-lived signed URLs instead of permanent public ones. Separately, a signed-in user could grant themselves administrator rights by editing their own profile record; that is now blocked at the database. Three migrations accompany this release.

_Full engineering detail for this release: `docs/BASAgents-fixes-2026-08-29.md`._
